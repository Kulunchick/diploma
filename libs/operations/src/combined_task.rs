...
impl CombinedTask {
    ...
    ///
    ///     (1 − r_ij) · p_ij / b_ij  ≥  s_ij
    ///
    /// У нашій моделі обсяг ресурсу IT-компанії b_ij використовується як проксі
    /// вартості провайдера (стаття записує (4) через витрати провайдера).
    /// Якщо b_ij = 0, знаменник вартості відсутній → пара тривіально допустима.
    pub fn admissible(&self, i: usize, j: usize, r: f64) -> bool {
        let b = self.b_ij[[i, j]];
        if b == 0 {
            return true;
        }
        (1.0 - r) * self.p_ij[[i, j]] as f64 / b as f64 >= self.s_ij[[i, j]]
    }
